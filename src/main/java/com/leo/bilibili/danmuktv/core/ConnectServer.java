...
public class ConnectServer {
	...
	/**
	 * @param _roomid
	 *            要连接的直播间的ID（真实ID，短ID无效）
	 * @param _uid
	 *            用户ID（随便写一个用户的ID就行了，自己的就行）
	 */
	...
	public InputStream conn() {
		...
		try {
			...
			byte[] msgLengthBytes = ByteUtil.intToByteArray(msgLength);
			byte[] argBetweenLengthAndAskarg = ByteUtil.intToByteArray(1048577);
			byte[] Askarg = ByteUtil.intToByteArray(7);
			byte[] argbehindAskarg = ByteUtil.intToByteArray(1);

			// 组合成发起连接包的头部
			byte[] sendHead = ByteUtil.merge(
					ByteUtil.merge(ByteUtil.merge(msgLengthBytes, argBetweenLengthAndAskarg), Askarg),
					argbehindAskarg);

			// 头部和主体拼接
			byte[] sendm = ByteUtil.merge(sendHead, send.getBytes("UTF-8"));
